...
export default function EditBooking() {
  ...
  useEffect(() => {
    ...
    if (formData.debitAmount && creditSettings && originalBooking) {
      validateCredit()
    }
  }, [formData.debitAmount, creditSettings, originalBooking])
  ...
  const fetchBookingData = async () => {
    try {
      ...
      setFormData({
        ...
        debitAmount: booking.ticketCost?.toString() || "",
        creditAmount: booking.amountReceived?.toString() || "",
        agent: booking.agent || "",
  ...
  const validateCredit = () => {
    const newTicketCost = Number.parseFloat(formData.debitAmount) || 0
    ...
    if (newTicketCost === originalTicketCost) {
      setCreditValidation({
        isValid: true,
        message: "No change in ticket cost",
        type: "success",
      })
      return
    }
    ...
    if (creditDifference > 0) {
      // Cost is increasing
      if (creditDifference > availableCredit) {
        setCreditValidation({
          isValid: false,
          message: `Insufficient credit! Additional amount ₹${creditDifference.toFixed(2)} exceeds available credit ₹${availableCredit.toFixed(2)}`,
          type: "error",
        })
      } else if (creditDifference > availableCredit * 0.8) {
        setCreditValidation({
          isValid: true,
          message: `Warning: Additional ₹${creditDifference.toFixed(2)} will be used. Remaining: ₹${(availableCredit - creditDifference).toFixed(2)}`,
          type: "warning",
        })
      } else {
        setCreditValidation({
          isValid: true,
          message: `Credit check passed. Additional ₹${creditDifference.toFixed(2)} will be used. Remaining: ₹${(availableCredit - creditDifference).toFixed(2)}`,
          type: "success",
        })
      }
    } else {
      ...
      setCreditValidation({
        isValid: true,
        message: `Credit will be freed: ₹${Math.abs(creditDifference).toFixed(2)}. New available credit: ₹${(availableCredit - creditDifference).toFixed(2)}`,
        type: "success",
      })
    }
  }
  ...
  const handleUpdateEntry = async () => {
    if (creditValidation && !creditValidation.isValid) {
      alert("Cannot update booking: " + creditValidation.message)
      return
    }
